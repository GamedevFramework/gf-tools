...
namespace gftools {
  ...
  TilesetScene::TilesetScene(TilesetApp& app)
  : gf::Scene(app.getRenderer().getSize())
  , m_app(app)
  , m_gui(app.datafile, app.data, app.random)
  {
...
//     addWorldEntity(m_display);
  ...
  bool TilesetScene::doEarlyProcessEvent(gf::Event& event) {
    return ImGui_ImplGF_ProcessEvent(event);
  }

  void TilesetScene::doUpdate(gf::Time time) {
    ImGui_ImplGF_Update(time);
  }

  void TilesetScene::doRender(gf::RenderTarget& target, const gf::RenderStates &states) {
    ImGui::NewFrame();
